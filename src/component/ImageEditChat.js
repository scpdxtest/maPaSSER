...
const T2ImgEdit = () => {
  ...
  const [useSDXL, setUseSDXL] = useState(false); // New state for SDXL toggle
  ...
  const handleGenerate = async () => {
    ...
    try {
      cleanupWebSocket();
      const wsEndpoint = useSDXL ? "/generate-sdxl" : "/generate";
  ...
  const handleEditImage = async () => {
    ...
    try {
      cleanupWebSocket();
      const wsEndpoint = useSDXL ? "/edit-sdxl" : "/edit";
      const ws = new WebSocket(getWebSocketUrl(wsEndpoint));
      wsRef.current = ws;
  ...
  const handleEditImageDirect = async () => {
    ...
    try {
      const imageBase64 = await convertImageToBase64(originalImage);
      const endpoint = useSDXL ? "/edit-sdxl-direct" : "/edit-direct";
  ...
  const getModelInfo = () => {
    ...
    const modelType = useSDXL ? "SDXL" : "FLUX";
    return portInfo ? `${portInfo.label} (${modelType})` : `Port ${selectedPort} (${modelType})`;
  };
          ...
          <Card title={`Image Editor - ${getModelInfo()}`}>
            <div className="p-fluid">
              ...
              <div className="p-grid">
                  ...
                  <div className="p-field">
                    ...
                    <Dropdown 
                      ...
                      onChange={(e) => { 
                        ...
                        } else if (e.value === "http://your-remote-server-ip") {
                          setSelectedPort(8281);
                        }
                      }} 
                ...
                <div className="p-col-4">
                  <div className="p-field">
                    <label htmlFor="useSDXL">Model Type</label>
                    <ToggleButton 
                      id="useSDXL"
                      checked={useSDXL} 
                      onChange={(e) => setUseSDXL(e.value)} 
                      onLabel="SDXL" 
                      offLabel="FLUX" 
                      onIcon="pi pi-bolt" 
                      offIcon="pi pi-image" 
                      className={useSDXL ? "p-button-success" : "p-button-primary"}
                      disabled={isLoading || isEditing}
                      style={{ width: "100%" }}
                    />
                  </div>
                ...
                <TabPanel header="Upload & Edit">
                  ...
                  <div className="p-field">
                    ...
                    <InputTextarea 
                      ...
                      rows={3} 
                      autoResize 
                      placeholder={useSDXL ? "SDXL: Change the hair color to blonde, same style" : "FLUX: Add a bright moon to the night sky"}
                      disabled={isEditing}
                    />
                  </div>
                  ...
                  <div className="p-field">
                    ...
                    <Slider 
                      ...
                      disabled={isEditing}
                    />
                    <small>{useSDXL ? "SDXL: 0.3-0.6 recommended for edits" : "FLUX: Lower = subtle, Higher = major changes"}</small>
                  </div>

                  {/* UPDATED: Edit buttons with integrated cancel functionality */}
                  <div className="p-field">
                    {isEditing ? (
                      <div className="p-grid" style={{ margin: 0 }}>
                        <div className="p-col-8" style={{ paddingRight: "0.25rem" }}>
                          <Button 
                            label={`Editing (${useSDXL ? 'SDXL' : 'FLUX'})... ${progress}%${remainingTime ? ` (ETA ${remainingTime})` : ""}`} 
                      ...
                      <div className="p-grid" style={{ margin: 0 }}>
                        <div className="p-col-6" style={{ paddingRight: "0.25rem" }}>
                          <Button 
                            label={`Edit (${useSDXL ? 'SDXL' : 'FLUX'})`} 
                            icon={useSDXL ? "pi pi-bolt" : "pi pi-pencil"} 
                            onClick={handleEditImage} 
                            className={useSDXL ? "p-button-success p-button-lg" : "p-button-warning p-button-lg"} 
                            style={{ width: "100%" }}
                ...
                <TabPanel header="Generate New">
                  <div className="p-field">
                    ...
                    <InputTextarea 
                      ...
                      rows={4} 
                      autoResize 
                      placeholder={useSDXL ? "SDXL: A photorealistic portrait of a person..." : "FLUX: A beautiful night sky with stars..."}
                      disabled={isLoading}
                    />
                  </div>

                  {/* UPDATED: Generate button with integrated cancel functionality */}
                  <div className="p-field">
                    {isLoading ? (
                      <div className="p-grid" style={{ margin: 0 }}>
                        <div className="p-col-8" style={{ paddingRight: "0.25rem" }}>
                          <Button 
                            label={`Generating (${useSDXL ? 'SDXL' : 'FLUX'})... ${progress}%${remainingTime ? ` (ETA ${remainingTime})` : ""}`} 
                            icon="pi pi-spin pi-spinner" 
                            className="p-button-primary p-button-lg" 
                            disabled={true} 
                            style={{ width: "100%" }} 
                      ...
                      <Button 
                        label={`Generate (${useSDXL ? 'SDXL' : 'FLUX'})`} 
                        icon={useSDXL ? "pi pi-bolt" : "pi pi-image"} 
                        onClick={handleGenerate} 
                        className={useSDXL ? "p-button-success p-button-lg" : "p-button-primary p-button-lg"} 
                        style={{ width: "100%" }}
                ...
                <TabPanel header="Settings">
                  <div className="p-field">
                    ...
                    <InputTextarea 
                      ...
                      rows={2} 
                      autoResize 
                      placeholder={useSDXL ? "SDXL: bad quality, blurry, distorted" : "FLUX: low quality, artifacts, noise"}
                      disabled={isLoading || isEditing}
                    />
                  </div>
                    ...
                    <div className="p-col-6">
                      <div className="p-field">
                        ...
                        <InputNumber 
                          ...
                          disabled={isLoading || isEditing}
                        />
                        <small>{useSDXL ? "SDXL: 25-50 recommended" : "FLUX: 20-30 recommended"}</small>
                      </div>
                    </div>
                    <div className="p-col-6">
                      <div className="p-field">
                        ...
                        <InputNumber 
                          ...
                          disabled={isLoading || isEditing}
                        />
                        <small>{useSDXL ? "SDXL: 7-12 recommended" : "FLUX: 7-10 recommended"}</small>
                      </div>
                    </div>
                  </div>
