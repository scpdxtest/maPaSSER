...
def signal_handler(sig, frame):
    """Handle Ctrl+C and systemd shutdown gracefully"""
...
signal.signal(signal.SIGTERM, signal_handler)  # ← Add this for systemd
...
def cancel_all_related_tasks(base_task_id: str):
    """Cancel a task and all its child agent tasks"""
    logger.info(f"🛑 Cancelling task family: {base_task_id}")
    
    # Cancel the main task
    cancellation_tracker.mark_cancelled(base_task_id)
    
    # Also cancel all potential agent/manager variations
    for suffix in ['_agent1', '_agent2', '_agent3', '_agent4', '_agent5', 
                   '_agent6', '_agent7', '_agent8', '_agent9', '_agent10',
                   '_manager']:
        agent_task_id = f"{base_task_id}{suffix}"
        cancellation_tracker.mark_cancelled(agent_task_id)
    
    logger.info(f"✅ Marked {base_task_id} and all child tasks for cancellation")
...
class CancellationStoppingCriteria(StoppingCriteria):
    ...
    def __init__(self, task_id: str):
        ...
        # Extract parent task_id if this is an agent task
        # Format: "ws_140538722645136_1760608707_agent1" → "ws_140538722645136_1760608707"
        if "_agent" in task_id or "_manager" in task_id:
            parts = task_id.rsplit("_", 1)  # Split from right, max 1 split
            if len(parts) == 2 and (parts[1].startswith("agent") or parts[1] == "manager"):
                self.parent_task_id = parts[0]
    ...
    def __call__(self, input_ids, scores, **kwargs):
        ...
        # Check cancellation every N tokens (not every token for performance)
        if self.token_count % self.check_interval == 0:
            # Check both agent-specific task_id AND parent WebSocket task_id
            if cancellation_tracker.is_cancelled(self.task_id):
                logger.warning(f"🛑 Generation stopped for task {self.task_id} at token {self.token_count}")
                return True
            
            # ALSO check parent task if this is an agent task
            if self.parent_task_id and cancellation_tracker.is_cancelled(self.parent_task_id):
                logger.warning(f"🛑 Generation stopped for parent task {self.parent_task_id} at token {self.token_count}")
                return True
...
class Agent:
    ...
    async def process_query(self, query: str, context: str, strategy: str, task_id: str = None, custom_context_size: Optional[int] = None) -> Dict:
        ...
        # Extract parent task_id for checking cancellation
        parent_task_id = None
        if "_agent" in self.task_id or "_manager" in self.task_id:
            parts = self.task_id.rsplit("_", 1)
            if len(parts) == 2 and (parts[1].startswith("agent") or parts[1] == "manager"):
                parent_task_id = parts[0]
        
        def is_cancelled():
            """Check if either agent task or parent WebSocket task is cancelled"""
            if cancellation_tracker.is_cancelled(self.task_id):
                return True
            if parent_task_id and cancellation_tracker.is_cancelled(parent_task_id):
                return True
            return False
    ...
    Answer:"""
            ...
            # Add cancellation stopping criteria
            ...
            # Track start time for long generation warnings
            generation_start = time.time()
            last_log_time = generation_start
            ...
            while not generation_task.done():
                if is_cancelled():
                    ...
                    # Mark task as cancelled to stop the StoppingCriteria
                    cancellation_tracker.mark_cancelled(self.task_id)
                    if parent_task_id:
                        cancellation_tracker.mark_cancelled(parent_task_id)
                    
                    # Cancel the async task wrapper
                    generation_task.cancel()
                    
                    # Wait a bit for generation to stop naturally via StoppingCriteria
                    try:
                        await asyncio.wait_for(generation_task, timeout=2.0)
                        logger.info(f"✅ Agent {self.agent_id} generation stopped gracefully")
                    except asyncio.TimeoutError:
                        logger.warning(f"⚠️ Agent {self.agent_id} generation didn't stop in time")
                    except asyncio.CancelledError:
                        logger.info(f"✅ Agent {self.agent_id} generation task cancelled")
                    
                    raise asyncio.CancelledError()
                
                # Log progress every 10 seconds for long generations
                current_time = time.time()
                if current_time - last_log_time >= 10:
                    elapsed = int(current_time - generation_start)
                    logger.info(f"⏳ Agent {self.agent_id} still generating... ({elapsed}s elapsed)")
            ...
            if is_cancelled():
                logger.warning(f"Agent {self.agent_id} generation was cancelled after completion")
                raise asyncio.CancelledError()
            ...
            if parent_task_id:
                cancellation_tracker.remove(parent_task_id)
            raise
        except Exception as e:
...
def create_agents(num_agents: int, strategy: str, model, tokenizer, device, 
    ...
    if strategy == "collaborative":
        ...
        # FIX: For hierarchical, Manager should be LAST, specialists first
        roles = [f"Specialist {i}" for i in range(1, 10)] + ["Manager"]
    else:
    ...
    for i in range(num_agents):
        ...
        agents.append(agent)
        logger.info(f"Created Agent {i+1} with role: {role}")
...
async def sequential_strategy(agents: List[Agent], query: str, context: str, 
                             ...
                             task_id: str = None,  # ← ADD THIS PARAMETER 
                             custom_context_size: Optional[int] = None,
                             _metrics: Dict = None, _agents_seen: set = None,
                             tokenizer = None) -> Dict:
    ...
    for idx, agent in enumerate(agents):
        ...
        # FIX: Pass task_id to agent
        response = await agent.process_query(
            query, accumulated_context, "sequential", 
            f"{task_id}_agent{agent.agent_id}" if task_id else None, 
            agent_context_size
        )
...
async def competitive_strategy(agents: List[Agent], query: str, context: str, 
    ...
    # FIX: Pass task_id to each agent
    tasks = [agent.process_query(query, context, "competitive", 
                                 f"{task_id}_agent{agent.agent_id}" if task_id else None, 
                                 custom_context_size) 
             for agent in agents]
...
async def hierarchical_strategy(agents: List[Agent], query: str, context: str, 
    ...
    # FIX: Pass task_id to specialists
    specialist_tasks = []
    for agent in specialists:
        task = asyncio.create_task(
            agent.process_query(query, context, "hierarchical", 
                              f"{task_id}_agent{agent.agent_id}" if task_id else None,
                              custom_context_size)
        )
        specialist_tasks.append(task)
        logger.info(f"✅ Started specialist task for Agent {agent.agent_id}")

    # Monitor specialist progress with keepalives
    valid_specialist_responses = []
    processed_tasks = set()  # Track which tasks we've already processed
    
    while len(valid_specialist_responses) < len(specialists):
        # Wait for any task to complete (with 5 second timeout)
        done, pending = await asyncio.wait(
            [t for t in specialist_tasks if t not in processed_tasks],
            timeout=5.0,
        ...
        for task in done:
            ...
            try:
                response = await task
                if not isinstance(response, (asyncio.CancelledError, Exception)):
                    valid_specialist_responses.append(response)
                    
                    # Find which agent this was
                    agent_id = response.get("agent_id", "?")
                    
                    # Send progress update
                    progress = 50 + int((len(valid_specialist_responses) / len(specialists)) * 15)
                    await safe_ws_send({
                        "type": "progress",
                        "progress": progress,
                        "message": f"Specialist {len(valid_specialist_responses)}/{len(specialists)} completed..."
                    })
                    logger.info(f"✅ Specialist Agent {agent_id} completed ({len(valid_specialist_responses)}/{len(specialists)})")
                else:
                    logger.error(f"❌ Specialist task failed with: {response}")
            except Exception as e:
                logger.error(f"❌ Error processing specialist task: {e}")
        
        # Send keepalive if still waiting
        if len(valid_specialist_responses) < len(specialists):
            await safe_ws_send({
                "type": "keepalive",
                "timestamp": int(time.time()),
                "message": f"Processing... ({len(valid_specialist_responses)}/{len(specialists)} specialists done)"
            })
            logger.debug(f"⏳ Waiting for specialists: {len(valid_specialist_responses)}/{len(specialists)} complete")
    
    logger.info(f"✅ All {len(specialists)} specialists completed")
    ...
    logger.info(f"✅ Starting manager task (Agent {manager.agent_id})...")
    
    # Monitor manager progress with periodic keepalives
    # FIX: Pass task_id to manager
    manager_task = asyncio.create_task(
        manager.process_query(query, manager_context, "hierarchical",
                            f"{task_id}_manager" if task_id else None,
                            manager_context_size)
    )
    
    # Poll manager task and send keepalives
    keepalive_counter = 0
    while not manager_task.done():
        ...
        if not manager_task.done():
            ...
            await safe_ws_send({
                "type": "keepalive",
                "timestamp": int(time.time()),
                "message": f"Manager synthesizing... ({keepalive_counter * 5}s elapsed)"
            })
            
            # Update progress bar slowly
            progress = min(75, 65 + keepalive_counter)
            await safe_ws_send({
                "type": "progress",
                "progress": progress,
                "message": "Manager analyzing and synthesizing..."
            })
            
            logger.debug(f"⏳ Manager still working... ({keepalive_counter * 5}s elapsed)")
    
    # Get manager result
    try:
        manager_response = await manager_task
        logger.info(f"✅ Manager Agent {manager.agent_id} completed")
    except Exception as e:
    ...
    logger.info(f"✅ Hierarchical strategy complete: {len(all_responses)} total responses")
...
async def process_multiagent_request(data: dict, websocket: WebSocket = None, 
    ...
    # Use provided task_id instead of creating new one
    if not task_id:
        task_id = f"ws_{id(websocket)}_{int(time.time())}" if websocket else f"task_{int(time.time())}"
    ...
    async def safe_send(message_dict):
        ...
        if websocket and (ws_active_ref is None or ws_active_ref[0]):
            try:
                ...
                # Mark task as cancelled if WebSocket fails
                cancel_all_related_tasks(task_id)  # ← USE NEW HELPER
                return False
        return False
    ...
    try:
        ...
        logger.info(f"🎯 MultiAgent request (task_id: {task_id}): {num_agents} agents, "
        ...
        # Execute strategy - FIX: Pass task_id instead of None
        _t_gen = _now_ms()
        ...
        logger.warning(f"🛑 Processing cancelled (task_id: {task_id})")
        cancellation_tracker.mark_cancelled(task_id)
        raise
...
async def multiagent_query_endpoint(websocket: WebSocket):
    ...
    message_handler_task: Task = None
    processing_task: Task = None
    
    # ✅ CREATE TASK ID ONCE AT THE START
    ...
    async def handle_incoming_messages():
        """Handle incoming messages (pings) from client"""
        try:
            while ws_active[0]:
                try:
                    ...
                    logger.warning(f"🛑 Client disconnected - cancelling task {task_id}")
                    ws_active[0] = False
                    cancel_all_related_tasks(task_id)
                    break
                except Exception as e:
    ...
    async def monitor_connection():
        ...
        try:
            while ws_active[0]:
                ...
                time_since = int(time.time() - last_activity[0])
                
                # Only timeout if no activity for 5 minutes (300s)
                if time_since > 300:
                    logger.warning(f"⏰ No activity for {time_since}s, closing connection")
                    ws_active[0] = False
                    cancel_all_related_tasks(task_id)
                    break
                else:
                    logger.debug(f"💚 Connection healthy (last activity: {time_since}s ago)")
    ...
    try:
        # Get initial request BEFORE starting message handler
        logger.info("⏳ Waiting for client to send initial request data...")
        ...
        try:
            ...
            logger.error("❌ Timeout waiting for initial message (30s)")
            ws_active[0] = False
        ...
        logger.info(f"✅ Request validated, starting background tasks...")
        
        # NOW start message handler (after initial request received)
        message_handler_task = asyncio.create_task(handle_incoming_messages())
        logger.info("✅ Message handler started")
        
        # Start connection monitor
        keepalive_task = asyncio.create_task(monitor_connection())
        logger.info("✅ Connection monitor started")
        
        logger.info(f"✅ Starting processing with task_id: {task_id}")
        
        # ✅ PASS TASK_ID TO process_multiagent_request
        processing_task = asyncio.create_task(
            process_multiagent_request(data, websocket, ws_active, last_activity, task_id)
        )
        ...
        try:
            ...
            logger.warning(f"Processing task cancelled (task_id: {task_id})")
            cancel_all_related_tasks(task_id)
        except Exception as e:
            logger.error(f"Processing task error: {e}")
        
    except WebSocketDisconnect:
        logger.warning(f"WebSocket disconnected by client (task_id: {task_id})")
        ws_active[0] = False
        cancel_all_related_tasks(task_id)
    except Exception as e:
        ...
        for task in [keepalive_task, message_handler_task, processing_task]:
...
if __name__ == "__main__":
    ...
    is_systemd = os.getenv('INVOCATION_ID') is not None or sys.stdin is None or not sys.stdin.isatty()
    
    if is_systemd:
        logger.info("🔧 Running under systemd")
    ...
    logger.info(f"🛑 Cancellation: ENABLED")
    
    try:
        # Configure uvicorn with systemd-friendly settings
        config = uvicorn.Config(
            ...
            log_level="info",
            loop="asyncio",
            timeout_keep_alive=120,  # Longer keepalive for systemd
            timeout_notify=30,        # Watchdog timeout
