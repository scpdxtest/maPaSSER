...
const TestMultiAgentBatch = () => {
    ...
    const [contextSize, setContextSize] = useState(null); // null = auto
    ...
    const cancelRef = useRef({ current: false }); // FIXED: Cancellation ref
    ...
    const processQuestionWithMultiAgent = async (question, context = '') => {
        return new Promise((resolve, reject) => {
            ...
            ws.onopen = () => {
                ...
                // Set 2-hour timeout (7200000ms)
                timeoutRef.current = setTimeout(() => {
                    if (ws.readyState === WebSocket.OPEN || ws.readyState === WebSocket.CONNECTING) {
                        console.warn('⏰ Request timeout after 2 hours');
                        ws.close();
            ...
            ws.onmessage = (event) => {
                ...
                if (data.type === 'progress') {
                    ...
                    if (timeoutRef.current) {
                        ...
                        timeoutRef.current = setTimeout(() => {
                            if (ws.readyState === WebSocket.OPEN || ws.readyState === WebSocket.CONNECTING) {
                                console.warn('⏰ Request timeout - no progress updates for 1 hour');
                                ws.close();
                                reject(new Error('Request timeout - no progress updates for 1 hour'));
                            }
                        }, 3600000);
                    }
            ...
            ws.onclose = (event) => {
                ...
                if (result) {
                    ...
                    // Normal closure without result - might be a server issue
    ...
    const cancelBatchTest = () => {
        ...
        // FIXED: Mark cancellation in ref
    ...
    const startBatchTest = async () => {
        ...
        for (let i = 0; i < QAJSON.length; i++) {
            // FIXED: Check cancel ref instead of state
            ...
            while (attempt <= MAX_RETRIES && !success && !cancelRef.current.current) {
                try {
                    ...
                    // Step 1: Get MultiAgent response
                    setResults(prevResults => 
                        prevResults + 
                    ...
                    try {
                        ...
                        if (response.data && response.data.metrics) {
                            ...
                            // Update results with full information
                            setResults(prevResults => 
                                prevResults.replace(
                                    `${i + 1}/${QAJSON.length} -> Question: ${question}\nReference: ${referenceAnswer}\nStatus: 📊 Calculating metrics...\n`,
                                    `${i + 1}/${QAJSON.length} -> Question: ${question}\n` +
        ...
        // FIXED: Check if completed all questions (not cancelled)
                ...
                <Card title="⚙️ MultiAgent Batch Test Configuration" className="config-card">
                    ...
                    {isTesting && (
                        <Message 
                            ...
                            text="⏱️ Timeout: 1 hour per question. Each progress update resets the timer."
                        />
                    )}
                </Card>
            </div>
