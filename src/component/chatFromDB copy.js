...
const ChatFromDB = () => {
    ...
    const embeddings_open = new OllamaEmbeddings({
        model: selectedEmbeddingModel, // Use embedding model
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const embeddings = await mdl.embeddings({ model: init_mdl, prompt: "test" });
                console.log("embeddings", embeddings);
                setSelectedEmbeddingModel(init_mdl);
            } catch (e) {
                if (e.message.includes("does not support embeddings")) {
                // Model does NOT support embeddings
                    console.error("llm embedding error: this model does not support embeddings", e);
    ...
    const handleSubmit = async () => {
        ...
        if (localStorage.getItem("retriever") === 'Normal') {
            ...
            retriever = ScoreThresholdRetriever.fromVectorStore(vectorStore1, {
                ...
                k: 100, // How many chunks to fetch at a time
            });
        }    
    // Get and log retrieved documents BEFORE passing to the chain
        const retrievedDocs = await retriever.getRelevantDocuments(question);
        console.log("Number of chunks retrieved:", retrievedDocs.length);
        console.log("Retrieved chunks:", retrievedDocs);
        ...
        // const docs = await vectorStore1.similaritySearch(question);
